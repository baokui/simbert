...
class TotalLoss(Loss):
    ...
    def compute_loss_of_similarity(self, inputs, mask=None):
        ...
        similarities = similarities - K.eye(K.shape(y_pred)[0])
        pos0 = similarities * y_true # 对称矩阵，不需要转置
        neg0 = (K.exp(similarities)) * y_true_inv
        pos = K.sum(pos0,axis=-1)
        neg = K.sum(neg0,axis=-1)
        loss = K.log(neg) - pos
        return loss
